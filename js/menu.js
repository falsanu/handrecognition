$(document).ready(function() {
	...
	var angle = 120;
	var distance = 90;
	var startingAngle = 270 + (-angle / 2);
	var slice = angle / (menuItemNum - 1);
	TweenMax.globalTimeScale(0.8);

	function getStartingAngle() {
		if (document.getElementById('wrap').classList.contains('left')) {
			console.log(document.getElementById('wrap').classList.contains('left'));
			return 270 + (-angle / 2);;
		}
		return 90 + (-angle / 2);;
	}
	...
	function updateCSS() {
		$(".menu-item").each(function(i) {
			var angle = getStartingAngle() + (slice * i);
			$(this).css({
	...
	$(".menu-toggle-button").mousedown(function() {
		TweenMax.to($(".menu-toggle-icon"), 0.1, {scale: 0.65})
	})
	$(document).mouseup(function() {
		TweenMax.to($(".menu-toggle-icon"), 0.1, {scale: 1})
	});
